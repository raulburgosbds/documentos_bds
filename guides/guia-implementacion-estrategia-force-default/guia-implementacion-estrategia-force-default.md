...
```mermaid
flowchart TD
    A[Detectar conflictos] --> B{¿Hay conflictos?}
    B -->|Sí| C[Lanzar ConflictException]
    B -->|No| D[Guardar nueva entidad]
...
```mermaid
flowchart TD
    A[1. Validar request] --> B[2. Buscar person]
    B --> C[3. Buscar certification type]
    C --> D[4. Construir nueva entidad]
    D --> E[5. Obtener certificaciones existentes]
    E --> F{6. mode == FORCE?}
    F -->|Sí| G[Usar ForceStrategy]
    F -->|No| H[Usar DefaultStrategy]
    G --> I[7. Aplicar estrategia]
    H --> I
    I --> J[8. Retornar ID guardado]
...
```mermaid
graph TD
    A[PersistenceStrategy<br/>Interface] --> B[DefaultStrategy<br/>Rechazar]
    A --> C[ForceStrategy<br/>Soft-delete]
